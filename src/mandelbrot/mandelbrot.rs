...
pub fn exec_mandelbrot(filename: &str, dimension: &str, upper_left: &str, lower_right: &str) {
    if filename.is_empty()
        || dimension.is_empty()
        || upper_left.is_empty()
        || lower_right.is_empty()
    {
    ...
    let bounds = parse_pair(dimension, 'x').expect("error parsing image dimensions");

    let upper_left = parse_complex(upper_left).expect("error parsing upper left corner point");

    let lower_right = parse_complex(lower_right).expect("error parsing lower right corner point");
