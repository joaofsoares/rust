...
pub fn sublist(first_list: &[i32], second_list: &[i32]) -> Comparison {
    let first_list_size: usize = first_list.len();
    let second_list_size: usize = second_list.len();
    let zero: usize = 0;

    if first_list_size == second_list_size {
        let mut cnt: usize = 0;
        while cnt < first_list_size {
            if first_list[cnt] != second_list[cnt] {
                return Comparison::Unequal;
            }
            cnt += 1;
        }
        return Comparison::Equal;
    } else if first_list_size > zero && second_list_size == zero {
